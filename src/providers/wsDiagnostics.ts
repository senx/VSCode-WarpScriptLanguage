...
export default class WSDiagnostics {
	...
	private getRangeFromStatementNumber(line: string, lineNo: number, statementNumber: number) {
		...
		let i = 0;
		while (newstr[i] == ' ' && i < newstr.length) { i++; } // first space are ignored (trim)
		for (; i < newstr.length; i++) {
			if (newstr[i] != ' ') {
				stmtStart = i;
				while (newstr[i] != ' ' && i < newstr.length) { i++; }
	...
	public initializeDiagnostics() {
		...
		this.context.subscriptions.push(workspace.onDidChangeConfiguration(_e => { this.refreshConfig(); }));
